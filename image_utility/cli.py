...
def i(model: str, input: IO, output: IO, **kwargs) -> None:
    output.write(remove_helper(input.read(), session=new_session(model), **kwargs))
...
def s(port: int, log_level: str) -> None:
    ...
    def im_without_bg(content: bytes, commons: CommonQueryParams) -> Response:
        return Response(
            remove_helper(content, alpha_matting=commons.a, alpha_matting_foreground_threshold=commons.af,
                          alpha_matting_background_threshold=commons.ab, alpha_matting_erode_size=commons.ae,
                          session=sessions.setdefault(
                                  commons.model.value, new_session(commons.model.value)
                              ), only_mask=commons.om, post_process_mask=commons.ppm),
